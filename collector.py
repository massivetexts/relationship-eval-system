...
def get_htid_list():
    files = glob.glob(os.path.join(app.root_path, 'data', '*'))
    clean_htids = [os.path.split(file)[1].replace('.json', '') for file in files]

    return [id_decode(htid) for htid in clean_htids]

@app.route("/optimize")
def optimize(pathglob='results/results.*.csv'):
    df, rater_targets = load_results()

    old_len = df.shape[0]
    df = df.drop_duplicates(['rater', 'target', 'candidate', 'judgment'])
    new_len = df.shape[0]

    old_files = glob.glob(os.path.join(app.root_path, pathglob))
    response = "<h2> Rewriting data files</h2><br />"
    prefix = 'old.'+str(int(time.time()))+'.'
    for old_file in old_files:
        path, filename = os.path.split(old_file)
        new_file = os.path.join(path, prefix + filename)
        os.rename(old_file, new_file)

    df.to_csv(os.path.join(app.root_path, 'results', 'results.0.csv'), mode='a', header=False)

    response += "Original size: {}, Deduplicated size: {}".format(old_len, new_len)
    return response
...
def submit():
    ...
    response = ""
    if not os.path.exists(os.path.join(app.root_path, 'results')):
        os.mkdir(os.path.join(app.root_path, 'results'))

    if len(input) > 1:
        try:
            formatted = request_as_df(input)
            formatted.to_csv(os.path.join(app.root_path, 'results', 'results.0.csv'), mode='a', header=False)
